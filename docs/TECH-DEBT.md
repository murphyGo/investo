...
- **Remaining (sub-bullets — re-evaluate when live API access for the deferred surfaces is restored)**:
  - **`fred-economic-calendar`** — FRED `releases/dates` / `release/dates` endpoint was unreachable from the u43 implementation network (TLS handshake to `api.stlouisfed.org` timed out across IPv4/IPv6/HTTP1.1/HTTP2 attempts; verified the existing `fred-macro` `series/observations` path was equally unreachable, so this is a transient FRED-side or network-side outage rather than a credential issue). Per R10 (no fabricated payloads), the adapter cannot land without a live recording. Re-attempt when FRED is reachable; the planned `INVESTO_FRED_CALENDAR_RELEASES` env-var override + `FRED_API_KEY` secret hygiene (already enrolled in `SECRET_ENV_VARS`) carry over from the original Suggested Fix.
...
- **Effort (remaining)**: ~2 h for `fred-economic-calendar` once FRED is reachable (live recording + adapter + tests); ~3 h for `coingecko-events` once a fallback aggregator + key is chosen; ~unknown for `krx-option-expiry` (depends on whether a public path is ever confirmed).
