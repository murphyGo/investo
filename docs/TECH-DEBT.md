...
- **Created**: 2026-07-19
- **Source**: DEBT-087 investigation (registry session 2026-07-19) — verified against commit `dc47537` and the live `site_docs/quality.md`
- **Reference**: FR-003 (static web publishing), NFR-003 (graceful degradation), NFR-005 (consistency / reader-trust), u10 / u22 coverage diagnostic, u31 source-health time series, u54 severity debounce, u69 quality-consistency gate, u137 R9 (`image_stage` note), DEBT-073 (this entry is the likely root cause of that entry's symptom)
- **Description**: `source_health.append_daily_coverage` writes one diagnostic line per run to `archive/_meta/coverage.jsonl` (default `_DEFAULT_COVERAGE_PATH = Path("archive/_meta/coverage.jsonl")`, overridable via `INVESTO_COVERAGE_LOG_PATH`). That path is **not** in the publisher's commit file list. The segmented publish stage stages exactly `archive_paths + asset_paths + macro_lineage_paths + index_paths + weekly_paths + extra_commit_paths` (`src/investo/orchestrator/pipeline.py`, the `commit_and_push` call) — `coverage.jsonl` is in none of those tuples. Verified three ways: (a) `git log --all -- archive/_meta/coverage.jsonl` is **empty** — the file has never been committed in the repo's history; (b) the most recent real bot commit `dc47537` ("briefing: 2026-07-18 segmented") stages `forecast_log.jsonl`, `quality_history.jsonl`, `run_traces/`, and `image_candidates/` under `archive/_meta/` but **not** `coverage.jsonl`; (c) `.gitignore` does *not* exclude it (`*.jsonl` is globally ignored but `!archive/**/*.jsonl` un-ignores it), so it is genuinely untracked-and-unstaged, not deliberately ignored. Nothing else persists it either: `daily-briefing.yml` sets only `INVESTO_OPERATOR_STATE_DIR: archive/_meta/operator_state` (a *different* path) and there is **no** `actions/cache` or artifact step in the workflow — the inline comment claiming the path "match[es] the path the workflow restores below" refers to a restore step that does not exist. Consequences, in order of severity:
  - **Public quality dashboard understates run health.** Every GHA run starts from a fresh checkout with no `coverage.jsonl`, so `compute_quality_kpis` sees at most the current run's own line. Live evidence on `main` — `site_docs/quality.md` renders `소스 라이브니스 0.0%` over a denominator of **`1 회`** while the archive-derived rows in the same table use **`11 건`** over the same 7-day window. A 0.0% liveness headline on a public page, computed from a 1-run sample, is a reader-trust defect.
  - **`detect_consecutive_failed` can never fire.** It requires a coverage line for *every* one of the last `DEFAULT_CONSECUTIVE_THRESHOLD` (3) days and returns `()` on any gap. With only today's line ever present, the "source failed N consecutive days" operator alert is structurally dead.
  - **u54 severity debounce can never open.** `should_alert_severity` has a documented first-run safety rule — "missing trailing history (no `coverage.jsonl` line for `today - 1`) → no alert" — which is now the permanent state, so severity-derived Telegram alerts to the operator never dispatch.
  - **u137's `image_stage` note is write-only.** The field is appended to the coverage line (`"image_stage": "ok: candidates=… indexed=… stored=…"` / `"failed: <ExceptionType>"`), which is exactly the "one coverage diagnostic line" the stage's failure-isolation contract (I16 / AC-137.4) points at — but since the line never lands in the repo it is observable only in the CI job log, which expires. The reporting half of that contract is effectively unshipped.
  - **Likely root cause of DEBT-073.** That entry records `coverage.jsonl` rows with `outcomes: []` producing a misleading `실패한 소스 누적 = 0` and treats it as historical residue needing a backfill. The non-persistence explains why such rows are the *normal* state rather than an anomaly; DEBT-073's repair pass will re-introduce the same symptom on the next run unless this entry is fixed first.
- **Suggested Fix**: Add the resolved coverage-log path to the segmented publish stage's `commit_and_push` file list — most cleanly as another `extra_commit_paths` contributor so it inherits u137 R9's existence-check-and-never-rollback semantics (the file is append-only diagnostics, so it must **not** join the rollback snapshot set). Two ordering details matter: (1) the coverage line is appended in a stage that runs *after* publish, so the line committed on day N is day N-1's — either accept the one-day lag and document it, or hoist the `append_daily_coverage` call ahead of the publish stage so the current run's own line lands in the same commit (preferred; it also makes the in-run KPI computation see the current run); (2) skip the add when `INVESTO_COVERAGE_LOG_PATH` points outside the repo, so operator-local runs don't try to stage a foreign path. Land DEBT-087 first or in the same change, otherwise a local `pytest` run will commit 81 synthetic test rows into the newly-tracked file. Pin with a regression test asserting the coverage path appears in the `git add` argv, mirroring the existing `*.svg.json` inclusion test. Re-run DEBT-073's replay afterwards — several of its flagged dates may resolve on their own once real history accumulates.
- **Effort**: ~1.5-2 h (commit-list wiring + stage-ordering decision + regression test + a short runbook note that the KPI window will read `n/a` until ~7 days of real history accumulate).
- **Priority Reasoning**: **Medium** — and the strongest of the current Mediums. Unlike most registry items this one has *verified, live, reader-facing* impact (`소스 라이브니스 0.0%` on the published quality page) plus two permanently-dead operator alert paths, and it silently negates work already paid for across u31 / u54 / u69 / u137. Held below High because it degrades *observability and reporting*, not the briefings themselves — no reader gets wrong market data, and the generated archive artifacts are unaffected; the fix is also small and well understood. **Promote to High** if an operator incident is traced to the dead consecutive-failure alert, or if the quality dashboard's understated liveness is cited by a reader. Should be sequenced **before** DEBT-073, which cannot hold its repair without this.

### Low Priority

#### DEBT-087: unit tests write into the real `archive/_meta/` — `coverage.jsonl` / `fact_snapshots.jsonl` appear untracked in every worktree after a test run

- **Created**: 2026-07-19
- **Source**: Registry session 2026-07-19 — stray untracked files observed in a fresh agent worktree
- **Reference**: NFR-006 (testing), NFR-007 (R10 — no fabrication), u31 source-health time series, u54 coverage severity schema, u137 R9 (`image_stage` coverage note), DEBT-088 (companion — the same file's production-side defect)
- **Description**: Parts of the unit suite exercise the coverage / fact-snapshot writers against their **real default paths** instead of `tmp_path`, so a plain `pytest` run leaves `archive/_meta/coverage.jsonl` and `archive/_meta/fact_snapshots.jsonl` behind as untracked files in whatever checkout the suite ran in. Observed in a clean worktree: 81 coverage rows and 21 fact-snapshot rows, none of them from any current work — the coverage rows all carry the synthetic test `target_date` `2026-04-27` with `outcomes: []` and severities `{domestic-equity: failed, us-equity: partial, crypto: failed}`, and 63 of the 81 carry a `image_stage` value (`ok: candidates=… indexed=… stored=…` / `failed: RuntimeError`) from the u137 R9 tests; the fact-snapshot rows carry recent wall-clock `observed_at` stamps (2026-07-16), confirming they are written on every run rather than being stale residue. Root cause for the coverage half is that `source_health._DEFAULT_COVERAGE_PATH` is a hardcoded literal `Path("archive/_meta/coverage.jsonl")` that is **not** derived from `investo.publisher.paths.ARCHIVE_ROOT`, so the suite's standard `monkeypatch.setattr("investo.publisher.paths.ARCHIVE_ROOT", …)` isolation fixture — used ~48 times in `tests/unit/orchestrator/test_run_pipeline.py` alone — does not redirect it; only tests that additionally `monkeypatch.chdir(tmp_path)` or pass `path=` explicitly are safe. `pipeline._fact_snapshot_path` *does* derive from `ARCHIVE_ROOT`, so its leak is narrower and comes from paths that leave `ARCHIVE_ROOT` at the real relative `archive/`. Two harms: (1) **repo pollution** — `git status` is dirty after every test run, which adds noise to every code review and to every agent worktree; (2) **latent data-clobbering risk** — an operator who runs the suite and then runs the pipeline locally has 81 synthetic rows sitting in the real KPI input, which the quality dashboard would consume as genuine history and publish (fabricated KPI figures, adjacent to the R10 no-fabrication rule). Production CI is not currently affected because tests and the daily briefing run in separate workflows on separate checkouts.
- **Suggested Fix**: Add an `autouse` fixture in `tests/conftest.py` (which today has no coverage/archive isolation at all) that points both writers at `tmp_path` for the whole suite — set `INVESTO_COVERAGE_LOG_PATH` and monkeypatch `investo.publisher.paths.ARCHIVE_ROOT` to a per-test temp dir — so isolation is opt-out rather than opt-in and no future test can regress it. Consider additionally deriving `_DEFAULT_COVERAGE_PATH` from `ARCHIVE_ROOT` (`ARCHIVE_ROOT / "_meta" / "coverage.jsonl"`) so the one documented archive seam redirects everything, keeping the `INVESTO_COVERAGE_LOG_PATH` env override as the outer layer. Delete the two stray files as part of the change. Optionally pin with a session-scoped check that the repo's `archive/_meta/` mtimes are unchanged after the suite runs.
- **Effort**: ~1 h (conftest fixture + the `_DEFAULT_COVERAGE_PATH` derivation + confirming no test depended on the real path).
- **Priority Reasoning**: **Low** — no production or reader-facing impact today; the cost is reviewer friction on every `git status` plus a latent local-only data risk that requires an operator to run tests and the pipeline in the same checkout. Kept out of Medium because the polluted files are untracked and therefore cannot reach the published archive on their own. **Promote to Medium** the moment DEBT-088 lands: once `coverage.jsonl` is added to the publisher's commit list, these synthetic rows stop being harmless untracked residue and become committable — a local publish after a test run would push fabricated coverage history to the public KPI surface. Sequencing note: land this **with or before** DEBT-088.
...
- **Stale-text correction (2026-07-19)**: The Description paragraph above was never amended after u43 and reads as if the whole feature is dormant. It is not. As of commit `f2b9cef` (2026-05-10, u43) the following are **landed and in production**, and none of them are open scope on this entry: (a) `fomc-calendar`; (b) `fred-economic-calendar`; (c) **M1** — the clock-explicit orchestrator wire-through (`_stage_notify_segmented_briefing` passes `now_utc` explicitly; `build_segmented_summary` raises `ValueError` when `lookahead_items_by_segment` is supplied with `now_utc=None`); (d) **M3** — the single-filter chokepoint `briefing.segments.filter_lookahead_items`, reused by both `_render_lookahead_context_block` and the orchestrator notify stage; (e) the `LOOKAHEAD_DATA_MISSING` reason code with its `LOOKAHEAD_AWARE_SOURCES` anti-regression guard. The two M1/M3 sub-bullets above are therefore **satisfied contracts, not pending work** — they are retained only as the rationale record for why the code is shaped that way. The Description's "four adapters" framing is also superseded on the numerator side: three lookahead adapters now ship (FOMC + FRED + `treasury-auctions`, see phase A below), and the original fourth (`coingecko-events`) is closed WONTFIX. Live scope on this entry is now exactly two externally-blocked items: the CoinGecko operator decision and `krx-option-expiry`.
- **Phase A resolution (2026-07-19 — commit `0699b69`, "DEBT-067 phase A: probe lookahead sources, land treasury-auctions")**: A live-access session probed all remaining lookahead surfaces and landed one new adapter.
  - **Landed — `treasury-auctions`**: the third lookahead adapter. Source: Fiscal Data `upcoming_auctions` dataset; Tier A; segment routing `us-equity`; category `calendar`; R10-compliant live fixtures **including a real empty-result recording** (the zero-row branch is pinned by a genuine upstream response, not a synthesized one); 26 unit tests. **Host selection was made on crawl-permissiveness, not data quality**: `www.treasurydirect.gov` carries the richer and fresher auction surface but serves `User-agent: * / Disallow: /`, so it was rejected; `api.fiscaldata.treasury.gov` serves `Allow: /` and was taken at a deliberate, accepted data-quality cost. **Default filter is coupon-only** (bills alone would flood the 12-row lookahead sub-cap and crowd out FOMC/FRED rows), overridable via `INVESTO_TREASURY_AUCTION_TYPES`.
  - **Closed WONTFIX — `coingecko-events`**: the endpoint is dead (HTTP 404, re-verified live 2026-07-19) and every free fallback is now closed: CoinMarketCal returns 403 without a registered key, and DefiLlama emissions — the previously-noted token-unlock fallback — now returns **402 Payment Required**, i.e. adopting it would violate the no-paid-APIs rule (NFR-002 / Critical Project Rule #4). **Recommendation recorded: drop crypto event lookahead entirely.** ⚠️ **This needs an operator decision** — register a CoinMarketCal free-tier key, or formally drop the surface. The dead-branch expectations cannot be pruned from the codebase until that decision is made, so the WONTFIX is *recommended*, not *ratified*.
  - **Still deferred — `krx-option-expiry`**: re-probed and still blocked. No public path exists: the data endpoint returns 403, `robots.txt` returns 404 (so crawl permission cannot even be established), and the portal HTML exposes no data endpoints. **The COMPUTE fallback is explicitly NOT actionable**: the "2nd Thursday" domestic option-expiry rule could not be verified against a live authoritative KRX page, and asserting it from model memory would be fabrication under R10 (NFR-007). This stays blocked until either KRX publishes a machine-readable path or an authoritative, fetchable page stating the expiry rule is located.
  - **Known caveat (`treasury-auctions` data freshness)**: the Fiscal Data dataset is an **accumulating archive with irregular refresh** — the landing snapshot spans 2024-03..2026-07 with long internal gaps. It is fresh as of landing, and the adapter's strict forward filter means a stalled upstream publisher degrades to **zero rows rather than stale rows** (fail-quiet, never wrong-data — consistent with NFR-003). Suggested follow-up: a freshness sentinel that alerts when the upstream max `record_date` lags the target date by ~10+ days, so a silent upstream stall is distinguishable from a genuinely empty auction calendar.
- **Suggested Fix**:
...
- **Effort (remaining)**: ~0 h of *engineering* work is currently unblocked. `coingecko-events`: ~15 min to prune the dead branch if the operator ratifies the drop, or ~3 h to build the adapter if the operator registers a CoinMarketCal key. `krx-option-expiry`: unknown, gated on an external precondition that does not exist today. Optional side task: ~1 h for the `treasury-auctions` freshness sentinel noted in the phase-A caveat.
- **Priority Reasoning**: **Demoted Medium → Low on 2026-07-19 (phase A re-triage).** Rationale: after phase A the remaining scope contains **no engineer-actionable item**. It is exactly (1) one item blocked on an external precondition that was re-probed and confirmed absent (`krx-option-expiry` — no public path, and the COMPUTE shortcut is barred by R10 rather than by effort), and (2) one item awaiting a **single operator decision** (CoinGecko: register a key vs formally drop), where the investigation is finished and the recommendation is already written down. Debt priority should track *what an engineer can pick up and burn down*; neither remainder qualifies, so carrying this at Medium overstates the queue and crowds the genuinely actionable Medium items. Meanwhile the payoff side is now well covered — three adapters (FOMC + FRED + `treasury-auctions`) populate the us-equity lookahead, so the reader-visible motivation that originally justified High/Medium is satisfied. Deliberately **not** closed, because both remainders are real coverage gaps with named unblock conditions. **Promotion triggers**: back to Medium the moment the operator registers a CoinMarketCal key (the adapter becomes a normal ~3 h task), or if KRX publishes a machine-readable path / an authoritative fetchable page stating the expiry rule is found; back to High only if a user-visible miss is traced to a missing crypto or domestic-derivatives event.
