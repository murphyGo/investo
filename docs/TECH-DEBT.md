...
- **Created**: 2026-05-24
- **Resolved**: 2026-09-08 — u151 Code Generation 6/6 local closeout. BundleContext now carries selected `fomc/oil/ust_yield` keys as a closed frozenset with sorted JSON projection; the cause-map maps typed keys rather than Korean display labels. Nonempty-block and independent allowlist gates, oil/Fed dedup/order and forbidden suppression are preserved; systemic risk remains dormant. Legacy empty-key blocks stay display-only. The common producer matcher also excludes exact CFTC positioning from shared candidates/core thesis without removing original delayed channel rows.
- **Evidence**: All six u151 ACs/five DoDs pass: full repository 5,204 tests in 304.88s, full static/policy/docs gate and independent cumulative 13-file review Pass; independent 394 focused tests and ten properties/500 generated examples pass. Relabel/legacy/forbidden behavior is covered through the actual three-segment finalizer. [Code summary](../aidlc-docs/construction/u151-shared-macro-positioning-kind-boundary/code/summary.md).
- **Boundary**: Resolved in the isolated working tree; cross-check, commit/push/main integration and production acceptance are still pending. No historical archive backfill or universal LLM-causality/sealed-input replay guarantee.
- **Source**: u74 market-channel-depth-v2 closeout (cause-map label-coupling risk)
- **Reference**: AC-74.5, NFR-005, u151 AC-151.1..6
...
- **Created**: 2026-07-19
- **Resolved**: 2026-07-19 — `append_daily_coverage` was **hoisted to the GenerateStage tail**, taking the entry's *preferred* no-lag option rather than the documented one-day lag: every input the call needs (`target_date`, `segmented_mode`, `items`, `source_outcomes`, `segment_briefings` **post entity-guard**, `image_stage_note`) is finalized before publish, so the run's own coverage line now ships in its own commit and the **in-run KPI computation sees it**. The resolved path flows `accumulated["coverage_log_path"] → _coverage_path_for_staging() → extra_commit_paths`, inheriting u137 R9's existence-check semantics and staying **out of the rollback snapshot set** — append-only diagnostics must never be reverted (DEBT-085 reasoning). One refinement vs the entry's suggested fix: the `ARCHIVE_ROOT`-derived default is in-repo *by construction* so it always stages; only an explicit `INVESTO_COVERAGE_LOG_PATH` override is subjected to the cwd-containment test (applying containment to the default breaks under tmp roots). The append itself stays best-effort (WARN, returns `None`). Pinned by 3 regression tests: the coverage path appears in the `git add` argv; a DEBT-085-shaped rollback survival (pre-seeded row **and** this run's row both intact after a forced `PublisherIOError`); an out-of-repo override is not staged. Two follow-on notes: the KPI window will read a short history until ~7 days of real lines accumulate, and **DEBT-073's replay should be re-run** now that this is fixed — its symptom was likely caused by this entry.
...
- **Reference**: FR-003 (static web publishing), NFR-003 (graceful degradation), NFR-005 (consistency / reader-trust), u10 / u22 coverage diagnostic, u31 source-health time series, u54 severity debounce, u69 quality-consistency gate, u137 R9 (`image_stage` note), DEBT-085 (rollback-exclusion precedent), DEBT-073 (likely downstream of this entry)
- **Priority Reasoning**: Closed.
...
- **Created**: 2026-07-19
- **Resolved**: 2026-07-19 — Three-part fix. (1) Production `resolve_coverage_path()` now derives from `publisher.paths.ARCHIVE_ROOT` **at call time** via the local-import seam and the frozen `_DEFAULT_COVERAGE_PATH` constant is gone — that is what makes the suite's standard `monkeypatch.setattr("investo.publisher.paths.ARCHIVE_ROOT", …)` actually redirect it. (2) A new **root `tests/conftest.py`** autouse fixture redirects `INVESTO_COVERAGE_LOG_PATH` / `INVESTO_FORECAST_LOG_PATH` / `INVESTO_QUALITY_HISTORY_PATH` to `tmp_path` **only when unset**, so per-test overrides still win; the env seam (rather than the `ARCHIVE_ROOT` monkeypatch) is required here because the forecast/quality writers live in `briefing/` and cannot import `publisher.paths` under module-boundary rule 3. (3) The orchestrator conftest pre-patches `ARCHIVE_ROOT` for the `fact_snapshots` leak. Verified: a full suite run now leaves **zero untracked files** under `archive/`. The stray residue was confirmed as test residue (81 rows, all `target_date` `2026-04-27` = the test constant, empty `outcomes`) and deleted.
- **Source**: Registry session 2026-07-19 — stray untracked files observed in a fresh agent worktree
- **Reference**: NFR-006 (testing), NFR-007 (R10 — no fabrication), u31 source-health time series, u54 coverage severity schema, u137 R9 (`image_stage` coverage note), DEBT-088 (companion — the same file's production-side defect), DEBT-089 (successor — the tracked-file half is not covered by this fix)
- **Priority Reasoning**: Closed.
