...
**Choice**: 기본 segmented production은 `publisher.public_document.finalize_public_bundle()`를 정확히 한 번 호출한다. 모든 Markdown producer와 deterministic repair는 E2 이전에 끝나며, terminal gate는 reader-visible label traversal과 canonical full-document surface scan을 포함한 read-only 경계다. 유효 문서는 `FinalizedPublicDocument` E5로 SHA-256 봉인되고 archive/index/OG/evidence/quality/replay/notifier consumer는 봉인된 bytes/DTO만 본다.
**Lifecycle order**: generated draft → 단일 typed producer plan(conditional payload와 region eligibility 공동 소유) → phase-one assembly(요약/nav/body evidence/watchpoint/visual·chart·carryover supplement) → 단일 public projection → outcome을 남기는 owned-region repair/replacement/omission → numeric/entity/compliance/structure/disclaimer/notification-summary read-only validation → E5 seal → E6 staged-artifact promotion/write/git. Seal 이후 `rendered_markdown` mutation은 architecture guard가 차단한다.
...
**Artifact transaction**: 선택된 staged artifact만 E5 ID와 E6 promotion manifest를 거쳐 공개 위치로 승격된다. git 전 I/O 실패는 기존 byte snapshot으로 rollback하며 기존 bytes는 shared atomic byte writer로 복원한다. 개별 restore/unlink가 실패해도 나머지를 시도한 뒤 `PublisherIOError`로 노출한다. commit/push 이후는 기존 `PublisherGitError` 복구 계약을 따른다.
**Result/notification contract**: `PipelineResult`의 `content_completeness`와 ordered segment outcomes가 exit 0/1/2 및 Pages sequencing의 근거다. notifier는 terminal layout과 E1 coverage에서 파생된 `PublicNotificationSummary`만 소비하며 generated `Briefing.market_summary` fallback은 없다. 공개 부분발행 문구는 `generation_absent`와 `trust_blocked`를 구분하되 issue code를 숨기고, 각 trust block은 별도 bounded publish-stage 운영자 경보를 만든다.
