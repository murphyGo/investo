...
def test_finalized_original_link_policy_keeps_good_siblings(
    ...
    # Raw link evidence reaches the original surface repair after reflow.
    # Recoverable unmatched syntax is cleaned there; canonical summary repair
    # can clean callout links. Unresolved required TL;DR links retain u144's
    # fallback-exhausted disposition, not u150's planned residual-code policy.
    assert observed
    if surface.startswith("tldr_") and code == "markdown.href_ellipsis":
        assert tuple(document.segment for document in result.documents) == (CRYPTO,)
        blocked = result.segment_outcomes[1]
        assert blocked.state == "trust_blocked"
        assert blocked.issue_codes == ("document.fallback_exhausted",)
    else:
