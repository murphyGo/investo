...
async def test_fetch_uses_latest_available_curve_on_or_before_target_date() -> None:
    ...
    assert item.published_at == datetime(2026, 5, 7, 20, 0, tzinfo=UTC)
...
async def test_january_fallback_uses_prior_year_when_current_year_has_only_future_rows() -> None:
    current_year = b"""
    ...
    <feed xmlns="http://www.w3.org/2005/Atom"
          ...
          xmlns:d="http://schemas.microsoft.com/ado/2007/08/dataservices">
      <entry><content><m:properties>
        <d:NEW_DATE>2025-12-31T00:00:00</d:NEW_DATE>
        <d:BC_3MONTH>4.45</d:BC_3MONTH>
        <d:BC_2YEAR>3.95</d:BC_2YEAR>
    ...
    async with _mock_client({"2026": current_year, "2025": previous_year}) as client:
        items = await adapter.fetch(client, window)

    assert len(items) == 1
    assert items[0].raw_metadata["date"] == "2025-12-31"
