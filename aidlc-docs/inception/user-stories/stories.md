...
## US-010 (← FR-022): 미국 섹터 core radar를 검증하고 안전하게 공개한다

**As a** Operator-User / Public Reader,
**I want** S&P 500의 11개 섹터 ETF를 SPY 대비 상대강도와 변화 방향으로 비교하고, 데이터가 공개 가능한지와 부족한 지표가 무엇인지 함께 확인하기를,
**So that** 현재 주도·둔화·회복·부진 섹터를 빠르게 파악하면서 NAV·거래량·실제 flow를 혼동하지 않도록.

**Personas**: P1, P2
**Acceptance Criteria**:
- [ ] 11개 섹터 ETF + SPY 고정 universe와 동일 as-of 기준을 사용한다.
- [ ] regime과 순위는 deterministic code로 계산하며 LLM이 수치나 상태를 만들지 않는다.
- [ ] 공개 OHLCV source가 확정되기 전에는 operator-provided local NAV workbook으로 private validation만 수행한다.
- [ ] private 화면은 NAV-only임을 명시하고 volume·거래대금·actual flow를 노출하지 않는다.
- [ ] public Pages는 사용권·63거래일 OHLCV·freshness·GHA 안정성 gate가 통과된 source만 사용한다.
- [ ] 실제 flow와 실적 actual은 Phase 2, Telegram은 웹 안정화 후 별도 범위로 유지한다.

**INVEST**: Independent (기존 briefing publish와 실패 격리) · Valuable (섹터 비교 신규 제품 가치) · Estimable · Testable (synthetic/private-schema fixtures + source qualification evidence)
...
| US-010 | FR-022 | NFR-002, NFR-003, NFR-006, NFR-008 |

NFR-006(Testing), NFR-007(Security), NFR-008(Data rights)은 별도 스토리화하지 않고 각 스토리의 AC와 코드 표준에 흡수.
