...
| FinancialData.Net ETF Prices API | **reject for public MVP under current published pricing and terms** | ETF별 10년+ 일봉 OHLCV, 300 records/response, SPY 예시 endpoint | 정확한 ETF Prices 경로는 무료가 아니라 Premium(월 USD 69/연 USD 599)이다. 외부 공개·재배포 권한은 Enterprise(월 USD 299/연 USD 2,599)에만 명시되며 Free의 Stock Prices는 회사 주식 경로이므로 12개 ETF 무과금 public radar 요구를 충족하지 못함 |
| HF Data Library daily OHLCV | **defer pending exact-universe and volume-fitness repair** | 무료 API, 23년+ 이력, 일봉 OHLCV, CC BY 4.0 + IEX attribution redistribution | 공개 catalog에 SPY와 10개 sector ETF만 있고 `XLRE`가 없어 11/12다. 2022-03 이후 데이터도 전체 통합거래량이 아니라 IEX 단일 거래소의 약 2–3%이며 무거래일·full-tape OHLC 차이가 가능하다. 공개 권리·비용은 유력하지만 현재 고정 universe와 총거래량 의미를 충족하지 못함 |
| BusinessQuant Stock Quotes API | **reject for public MVP under current terms and pricing** | 무료 API key, US-listed ETF 다년 EOD OHLCV, multi-ticker, Free 30 calls/day | 기술·비용 조건은 유력하지만 이용약관은 접근 데이터에 대한 소유권이나 라이선스를 부여하지 않는다. 가격표도 Commercial use API를 Enterprise에 두고 commercial redistribution을 플랜 기능으로 취급한다. 무료 public Pages 파생표시 권한이 명시되지 않아 계정·payload probe 전 탈락 |
| London Strategic Edge Free Market Data API | **reject for public MVP under current terms** | 무료 API key, daily JSON/CSV OHLCV, 5,000 rows/request, 2003년부터의 장기 이력, ETF 25개 | 약관이 데이터 재배포와 명시적 서면 동의 없는 복제·수정·배포·파생저작물 생성을 금지한다. 무료 API 접근이 public Pages 파생표시·보존 권한을 만들지 않으며 upstream ETF feed와 통합거래량 의미도 불명확하여 계정·payload probe 전 탈락 |
| Direct IEX Exchange HIST / TOPS | **reject for current public MVP operational and metric fitness** | 무료 T+1, 최근 12개월, attribution 조건부 배포 허용, 공식 TOPS feed | 종목별 일봉 API가 아니라 거래일별 전체시장 gzip PCAP이며 현재 공식 catalog 기준 일일 약 9–21GB다. 63일 bootstrap은 필터링 전 수백 GB와 IEX-TP decoder/일봉 집계가 필요하고, 산출 OHLCV도 통합시장이 아닌 IEX 단일 venue 체결만 의미하므로 현재 GHA public radar에 부적합 |
| MIAX Pearl Equities Historical Market Data | **reject under current published cost, delivery, and rights terms** | 최근 최대 6개월 ToM/DoM venue feed, last sale/order execution 포함 | 무료 API가 아니라 MIAX 제공 8TB USB 장치당 USD 500 구매이며 exchange data agreement와 request schedule이 필요하다. 자동 일봉 수집이 불가능하고 binary decode/cancellation 처리 후에도 MIAX 단일 venue OHLCV이므로 무과금 GHA public radar에 부적합 |
| Cboe DataShop Equity EOD Summary | **reject under current published cost and licensing terms** | 2010년 1월부터의 미국 주식·ETF 일봉 OHLC, trade volume, VWAP, 15:45/EOD bid/ask, 일별 CSV | historical purchase 또는 daily subscription 상품이며 영구 무료 entitlement가 없다. qualifying academic 할인도 50%·최소 USD 500이고 Investo에는 적용되지 않으며, Cboe agreement/policy/fee 및 external-distribution 라이선스 경계 아래 있어 무과금 public derived-display 권한이 확인되지 않음 |
| MEMX MEMOIR Historical Data | **reject for current public MVP cost/rights certainty and metric fitness** | cloud API로 제공되는 전일 MEMOIR Depth/Top/Last Sale 원시 메시지 | Market Data Agreement와 connectivity 요청, 승인된 Order Form/System Description이 필요하다. derived data도 사전 신고·승인·feed별 예외 대상이며 현재 Top/Last Sale 외부·digital-media 배포는 월 USD 2,000이다. 집계해도 MEMX 단일 venue OHLCV라 통합 거래량 radar에 부적합 |
| NYSE Daily TAQ | **reject under current published pricing, licensing, and operational budget** | 1993년부터의 CTA/UTP 통합 all-trades, all-quotes, NBBO T+1 원시 파일 | 기술적으로는 통합 ETF OHLCV 생성이 가능하지만 상용 구독이 월 USD 3,800·최소 12개월이고 구 이력과 외부 historical redistribution은 별도 과금/라이선스 대상이다. 일별 원시 파일도 Trades 약 649 MB, Quotes 약 17 GB, NBBO 약 2.2 GB 규모의 전 시장 자료라 경량 GHA 계약에 부적합 |
| NYSE TAQ Closing Prices | **reject under current published cost, public-rights, and volume-semantics terms** | NYSE 계열 거래소별 일봉 Open/High/Low/Last, Total Volume, closing bid/ask; NYSE Arca 2008년부터 | 원시 TAQ 처리 부담은 없지만 월 USD 500·최소 12개월이고 구 이력과 외부 historical distribution은 별도 과금/라이선스 대상이다. `Tot Vol`은 해당 NYSE 계열 거래소 체결량이므로 통합 미국 ETF 거래량이 아님 |
| SEC MIDAS Metrics by Individual Security | **reject as core OHLCV source; possible Phase 2 research input only** | 4,800개+ 종목의 일별 ticker/date, rank, trade/order count·volume, lit/odd-lot/hidden 지표 | 무키 공식 ZIP이지만 OHLC가 없고 분기 배포며 현재 2025년 12월에서 멈춘다. 거래량도 장중 on-exchange만 반영하고 장외·시가/종가 경매·시간외를 제외하여 통합 ETF 거래량이 아님 |
...
- FinancialData.Net ETF prices, pricing, and public-display boundary:
...
- HF Data Library daily bars, license, catalog, and IEX boundary:
...
- BusinessQuant free quotes API and rights boundary:
...
- London Strategic Edge free market-data API and rights boundary:
...
- Direct IEX Exchange HIST/TOPS delivery, terms, and protocol boundary:
...
- MIAX Pearl Equities historical feed, delivery, fees, and agreement boundary:
...
- Cboe DataShop Equity EOD Summary fields, delivery, pricing floor, and agreement boundary:
...
- MEMX MEMOIR historical delivery, feed semantics, fees, and approval boundary:
...
- NYSE Daily TAQ coverage, delivery, pricing, file scale, and license boundary:
...
- NYSE TAQ Closing Prices fields, delivery, pricing, and license boundary:
  ...
  <https://www.nyse.com/publicdocs/nyse/data/NYSE_Market_Data_Complete_Policy_Package.pdf>
  <https://www.nyse.com/market-data/pricing-policies-contracts-guidelines>
- SEC MIDAS individual-security schema, cadence, methodology, and volume boundary:
