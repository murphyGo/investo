...
- [x] `src/investo/sources/yahoo_finance_news.py` (또는 어댑터 파일 위치) 에 `_PERSONAL_FINANCE_DENY_PATTERNS: tuple[re.Pattern[str], ...]` 또는 substring tuple 신규 등록.
- [x] 패턴 (모두 case-insensitive 매치):
  - `cd rates`
  - `heloc`
...
- [x] Filter 적용 위치: RSS/JSON 파싱 후 `NormalizedItem` 생성 *이전* — Stage 1 LLM 이 아예 보지 못하게.
- [x] Filter 매치 시 INFO 로그 1줄 emit (배치 단위, item 단위 아님): `yahoo-finance-news: filtered N/M items as personal-finance noise (patterns: ...)` — N=차단 수, M=원래 fetch 수, patterns=hit 한 패턴 union (deduped). 향후 패턴 튜닝의 카나리.
- [x] 차단 비율 100% (M=N) 인 경우 WARNING 로그 추가 — feed 가 패턴 폭주하거나 (정상 시장 뉴스가 아예 없거나) deny list 가 너무 광범위한 신호; 운영자가 봐야 하는 surface.
- [x] R10 fixture (record/replay):
...
- [x] 회귀 테스트 (`tests/unit/sources/test_yahoo_finance_news_filter.py`):
...
- [x] **R10 invariant**: deny 패턴 fixture 는 fabricated 가 아닌 yahoo-finance-news 의 실제 record 에서 발췌. 정상 케이스도 마찬가지 — `tests/fixtures/sources/yahoo_finance_news/personal_finance_noise.json` 와 `..._market_signal.json` 두 fixture 파일에 분리.
- [x] 어댑터 외 변경 없음: `briefing/segments.py`, `orchestrator/`, `notifier/`, `publisher/` 미수정.
- [x] 전체 quality gate green: `ruff check` ✅, `ruff format --check` ✅, `mypy --strict src/` ✅, `pytest -q` ✅ (예상 +8-12 신규 테스트), `mkdocs build --strict` ✅.
...
- [x] `_PERSONAL_FINANCE_DENY_PATTERNS` 모듈 상수 정의 (substring tuple — 정규식보다 빠르고 디버깅 용이).
- [x] `_is_personal_finance_noise(title: str) -> tuple[str, ...]` 헬퍼 — 매치된 패턴 list 반환 (빈 tuple = 노이즈 아님). 매치 우선순위 없이 union 반환 (로그 카나리에 사용).
- [x] Filter 적용: `fetch()` 의 RSS/JSON parse loop 안, `NormalizedItem` 생성 *직전*.
- [x] 파일: `src/investo/sources/yahoo_finance_news.py` (또는 정확한 어댑터 파일 — sources/ ls 로 확인).

### Step 2 — Fixture 녹화 (R10)

- [x] 기존 yahoo-finance-news fixture 가 있다면 그 안의 노이즈 헤드라인 발췌하여 `personal_finance_noise.json` 생성 (또는 신규 record).
- [x] 정상 시장 뉴스 5개 fixture (record 또는 기존 fixture 발췌).
- [x] `_meta.json` sidecar (recorded_at + sha256).

### Step 3 — Filter + 로그 emit + 테스트

- [x] 어댑터 fetch loop 에 filter 통합.
- [x] INFO / WARNING 로그 emit (logging module — `redact_text(STRICT)` 적용 필요 없음; 패턴 매치는 secret-shaped 가 아님).
- [x] 회귀 테스트 작성.

### Step 4 — 검증

- [x] 전체 quality gate green.
- [x] (수동) 2026-05-08 fixture 재처리 시뮬레이션: 24건 중 ~10건이 차단되고 ~14건이 통과하는지 INFO 로그 확인.
...
- [x] `uv run pytest -q` ✅ (예상 +8-12 신규 테스트)
