...
**Status**: Complete (7/7) — cumulative quality gate and fresh-eyes review approved
...
1. **Value resolution keys** (segment → resolvable snapshot values, all read from payloads the orchestrator already passes to publish/reader-format):
   ...
   - Domestic: reconciled KR anchors only (quarantined anchors are never resolvable — u109/u130). The u67/u138 production shape is intentionally close-only; its trusted close resolves without inventing a percentage.
...
3. **Fallback trigger**: zero cards survive u110 filtering AND the segment payload resolves ≥1 key from contract 1. Then synthesize `min(2, resolvable signals)` cards in this fixed priority: (1) core anchor 52w/24h range card, or a domestic close-only reference card when u67/u138 has no derived range, (2) CFTC positioning divergence card (US/crypto), (3) F&G extreme card (crypto only, value ≤20 or ≥80).
4. **Closed Korean templates** (pinned; placeholders resolved from payload values, no free text):
   ...
   - Domestic close-only reference (Step 7 production-path correction): `관찰 신호: {label} 종가 기준선 · 현재: {close} · 확인 조건: 상방 {close} 상회 시 단기 회복 흐름 관찰; 하방 {close} 이탈 시 방어적 수급 관찰 · 신뢰도: 높음 · 관심 영향: 본문 §⑤ 가격 동향과 연계 점검.` It reuses the trusted close as an explicit future reference level and never fabricates a percentage or 52-week bound.
...
- [x] Step 7 — Quality gate: scoped ruff/format, `mypy src`, `pytest tests/unit/publisher tests/unit/orchestrator`, `mkdocs build --strict` if site_docs touched (expected: not touched).
...
- Rendered regressions from 2026-06-29 crypto and 2026-06-30 us-equity/domestic shapes, including the production close-only domestic anchor contract.
