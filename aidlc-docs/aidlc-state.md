...
| u19 briefing visual assets | ⏭️ SKIP initially (visual UX extension) | ⏭️ SKIP initially (no paid data, no external image scraping) | 🚧 In Progress (Step 1/4 complete) | Step 1 closed 2026-05-07: added the `investo.visuals` package contract, visual asset path helpers, strict card input models, external image policy, and unit tests. Plan: `aidlc-docs/construction/plans/u19-briefing-visual-assets-code-generation-plan.md`. |
