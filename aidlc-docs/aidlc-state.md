...
| u4 notifier | ⏭️ SKIP | ⏭️ SKIP | ⏳ in progress (CG plan approved 2026-04-30, 8 steps; Step 1 next) | FD + NFR both SKIP per execution-plan; consumes BriefingNotification + SendResult + FailureContext from models (already shipped); httpx already locked from u1; zero new deps. CLAUDE.md rule #5: BriefingPublisher + OperatorAlerter must NOT share chat_id (orchestrator-enforced + Step 6.2 dispatch pin) |
