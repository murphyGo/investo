...
| u19 briefing visual assets | ⏭️ SKIP initially (visual UX extension) | ⏭️ SKIP initially (no paid data, no external image scraping) | 🚧 In Progress (Step 2/4 complete) | Steps 1-2 closed 2026-05-07: added the `investo.visuals` contract, path helpers, strict card input models, external image policy, card builders, deterministic SVG renderer, and unit tests. Plan: `aidlc-docs/construction/plans/u19-briefing-visual-assets-code-generation-plan.md`. |
