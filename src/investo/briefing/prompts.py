...
- §⑥ 관전 매트릭스 구조 룰 (u72 watchpoint-action-matrix): §⑥ 의 각
  bullet 은 publisher 가 6열 관찰형 매트릭스
  (``관찰 신호 | 현재 | 상방 확인 조건 | 하방 확인 조건 | 신뢰도 |
  섹션 내 관심 영향``) 로 변환할 수 있도록 *source · 임계조건 ·
  관심 영향* 3요소를 모두 담아 작성한다. 권장 관찰형 템플릿:
  - ``{지표}가 {조건}을 확인하면 상방 압력 관찰`` (상방 확인 조건)
  - ``{지표}가 {조건}을 이탈하면 방어적 해석`` (하방 확인 조건)
  - ``관심 영향: 변동성 확대 여부를 점검`` (섹션 내 관심 영향)
  ...
  말고 "데이터 부족" 으로 명시한다 (publisher 가 ``데이터부족`` 행으로
  렌더). 금지 어휘 (관찰형 위반): ``매수 / 매도 / 비중 확대 / 목표가 /
  손절 / 진입 / 청산`` 및 결과를 단정하는 보장성 표현 — u56 게이트에서
  차단된다.
